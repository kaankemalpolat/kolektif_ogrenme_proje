...
def baggingClassifier_100vs10x10(X_train, X_test, y_train, y_test):
    ...
    t, p = paired_ttest_5x2cv(estimator1=eclf, estimator2=bagging, X=X_train, y=y_train, random_seed=56)
    t_test = pd.DataFrame([[t,p]], columns=['t','p'])


    result = pd.DataFrame([[bagging_acc,ens_acc]], columns=['bagging_acc','ens_acc'])
    
    return bagging, eclf, result, t_test
...
def adaBoostClassifier_100vs10x10(X_train, X_test, y_train, y_test):
    ...
    t, p = paired_ttest_5x2cv(estimator1=eclf, estimator2=ada, X=X_train, y=y_train, random_seed=56)
    t_test = pd.DataFrame([[t,p]], columns=['t','p'])

    result = pd.DataFrame([[ada_acc,ens_acc]], columns=['ada_acc','ens_acc'])

    return ada, eclf, result, t_test
...
def randomSubspaceClassifier_100vs10x10(X_train, X_test, y_train, y_test):
    ...
    t, p = paired_ttest_5x2cv(estimator1=eclf, estimator2=rnd_subspace, X=X_train, y=y_train, random_seed=56)
    t_test = pd.DataFrame([[t,p]], columns=['t','p'])        

    result = pd.DataFrame([[rnd_subspace_acc,ens_acc]], columns=['rnd_subspace_acc','ens_acc'])

    return rnd_subspace, eclf, result, t_test
...
def randomForestClassifier_100vs10x10(X_train, X_test, y_train, y_test):
    ...
    t, p = paired_ttest_5x2cv(estimator1=eclf, estimator2=rf, X=X_train, y=y_train, random_seed=56)
    t_test = pd.DataFrame([[t,p]], columns=['t','p'])

    result = pd.DataFrame([[rf_acc,ens_acc]], columns=['rf_acc','ens_acc'])
    
    return rf, eclf, result, t_test
...
def extraTreesClassifier_100vs10x10(X_train, X_test, y_train, y_test):
    ...
    t, p = paired_ttest_5x2cv(estimator1=eclf, estimator2=extra, X=X_train, y=y_train, random_seed=56)
    t_test = pd.DataFrame([[t,p]], columns=['t','p'])

    result = pd.DataFrame([[extra_acc,ens_acc]], columns=['extra_acc','ens_acc'])

    return extra, eclf, result, t_test
